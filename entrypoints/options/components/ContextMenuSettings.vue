    ...
    <div class="card-body">
      ...
      >
        <div 
          ...
          style="display: flex; align-items: center; justify-content: space-between; transition: background 0.2s, box-shadow 0.2s;"
          :style="[
            item.id.startsWith('divider_') 
              ? { padding: '2px 8px', background: 'transparent', border: 'none' }
              : { padding: '10px 14px', background: 'var(--bg-primary)', border: '1px solid var(--border-subtle)', borderRadius: '8px' },
        ...
        >
          <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
            ...
            <div v-if="item.id.startsWith('divider_')" style="flex: 1; display: flex; align-items: center; gap: 12px; margin-right: 12px; color: var(--text-tertiary);">
              <div style="flex: 1; height: 1px; background: var(--text-tertiary); opacity: 0.3;"></div>
              <div style="font-size: 12px; user-select: none; white-space: nowrap;">{{ getMenuLabel(item.id) }}</div>
              <div style="flex: 1; height: 1px; background: var(--text-tertiary); opacity: 0.3;"></div>
            </div>
            <template v-else>
          ...
          <div style="display: flex; align-items: center; justify-content: center;">
            <input 
              v-if="!item.id.startsWith('divider_')"
              type="checkbox" 
            ...
            <button 
              v-else
              @click="removeMenuConfigItem(item.id)" 
              class="btn btn-secondary" 
              style="padding: 4px; color: var(--text-tertiary); border-color: transparent; cursor: pointer;" 
      ...
      <div style="display: flex; justify-content: flex-end; margin-top: 12px; margin-right: 4px;">
        <button 
          class="btn" 
          @click="addDivider"
          style="display: flex; align-items: center; gap: 6px; padding: 6px 12px; font-size: 13px; color: var(--text-secondary); background: transparent; border: 1px dashed var(--border-subtle); border-radius: 6px; cursor: pointer;"
        >
          ...
          {{ t('context_menu.add_divider') }}
        </button>
      </div>

      <!-- Custom Search Engines -->
      <div v-show="settings.enableCustomContextMenu" style="margin-top: 24px; border-top: 1px solid var(--border-subtle); padding-top: 16px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <div v-for="(engine, index) in (Array.isArray(settings.customSearchEngines) ? settings.customSearchEngines : [])" :key="index" 
               style="display: flex; gap: 12px; align-items: center; padding: 12px 14px; background: var(--bg-primary); border: 1px solid var(--border-subtle); border-radius: 8px; transition: all 0.2s;"
          ...
          <button @click="addCustomEngine" class="btn" style="align-self: flex-start; margin-top: 4px; padding: 6px 12px; display: flex; align-items: center; gap: 6px; background: transparent; border: 1px dashed var(--border-subtle); color: var(--text-secondary); box-shadow: none; border-radius: 6px;">
...
const addDivider = () => {
  if (!Array.isArray(settings.customMenuConfig)) {
    settings.customMenuConfig = [];
  }
  settings.customMenuConfig.push({ id: 'divider_' + Date.now(), enabled: true });
  saveSettings();
};

const removeMenuConfigItem = (id: string) => {
  if (Array.isArray(settings.customMenuConfig)) {
    settings.customMenuConfig = settings.customMenuConfig.filter(item => item.id !== id);
    saveSettings();
  }
};
...
const getMenuLabel = (id: string) => {
  const map: Record<string, string> = {
    ...
    machine_translate: t('context_menu.machine_translate'),
    ...
    x: t('context_menu.x'),
    settings: t('context_menu.settings')
  };
  if (id.startsWith('divider_')) return t('context_menu.divider');
  return map[id] || id;
};
...
const getMenuIcon = (id: string) => {
  const map: Record<string, string> = {
    ...
    machine_translate: '<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M5 8l6 6"/><path d="M4 14l6 -6 2 -3"/><path d="M2 5h12"/><path d="M7 2h1"/><path d="M22 22l-5 -10 -5 10"/><path d="M14 18h6"/></svg>',
  ...
  if (id.startsWith('divider_')) return '<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none"><line x1="4" y1="12" x2="20" y2="12" stroke-dasharray="2 2"></line></svg>';
